#!/usr/bin/env python3
"""
Drive straight until LIDAR detects a wall within WALL_STOP_DIST, then stop
and publish /wall_reached. Alignment is handled by turn_controller.
"""
...
WALL_STOP_DIST = 0.7    # m   — stop when wall is this close
FRONT_HALF_ANG = 20.0   # deg — cone in front to measure distance
...
class WallFollower(Node):
    def __init__(self):
        ...
        self.create_subscription(Bool,      '/wall_follower/enable', self._enable_cb, 10)
    ...
    def _scan_cb(self, msg: LaserScan):
        ...
        for i, r in enumerate(msg.ranges):
            ...
            if abs(a_deg) <= FRONT_HALF_ANG and math.isfinite(r) and r > 0.05:
                ranges.append(r)
        self._front_dist = min(ranges) if ranges else float('inf')
    ...
    def _control_loop(self):
        ...
        if self._front_dist <= WALL_STOP_DIST:
            self._publish_cmd(0.0, 0.0)
            self._active = False
            self.get_logger().info(f'Wall reached at {self._front_dist:.2f} m')
            reached = Bool(); reached.data = True
